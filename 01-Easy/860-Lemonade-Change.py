...
Difficulty: Easy
Description: At a lemonade stand, each lemonade costs $5. Customers are standing in a queue to buy
from you and order one at a time (in the order specified by bills). Each customer will only buy one
...
class Solution:
    ...
    def lemonadeChange(bills: List[int]) -> bool:
        ...
        for bill in bills:
            ...
            if bill == 5:
                five += 1
            # If the bill is $10, increment the number of $10 bills
            # and decrement the number of $5 bills
            elif bill == 10:
                ten += 1
                five -= 1
            # If the bill is $20, try to give change with one $10 bill and one $5 bill,
            # otherwise, give change with three $5 bills
            else:
                if ten > 0:
                    ten -= 1
                    five -= 1
