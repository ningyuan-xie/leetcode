...
class Solution:
    ...
    def allCellsDistOrder(rows: int, cols: int, rCenter: int, cCenter: int) -> List[List[int]]:
        """Optimal Solution: Sorting with lambda function. Time Complexity: O(n*m), Space Complexity: O(n*m)."""
...
def unit_tests():
    ...
    assert Solution.allCellsDistOrder(2, 2, 0, 1) == [[0, 1], [0, 0], [1, 1], [1, 0]]

    # Input: rows = 2, cols = 3, rCenter = 1, cCenter = 2, Output: [[1, 2], [0, 2], [1, 1], [0, 1], [1, 0], [0, 0]]
    assert Solution.allCellsDistOrder(2, 3, 1, 2) == [[1, 2], [0, 2], [1, 1], [0, 1], [1, 0], [0, 0]]
